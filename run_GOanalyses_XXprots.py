...
def main(args):
    ...
    prots_df = get_prots(gaf_df, lcd_class)
    ...
    for proteome in gaf_df:
        ...
        assoc = get_assoc(lcd_class, gaf_file)
...
def get_gaf_df(lcd_class):
    ...
    h = open(os.path.join('GOAfiles', 'GOA_FileMap_' + lcd_class + '_LCDs.tsv'))
    df = {}
    df2 = {}
    h.readline()
...
def get_prots(gaf_df, lcd_class):

    h = open(os.path.join('SecondaryLCDs_by_LCDcategory', lcd_class + '_LCDs_AllOrganisms.tsv'))
    header = h.readline()
...
def get_assoc(lcd_class, gaf_file):
    """
    ...
    h = open(os.path.join('GOAfiles', lcd_class + '_GOanalysis', gaf_file))
