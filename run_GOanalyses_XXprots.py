...
def main(args):
    ...
    cwd = os.getcwd()
    lcd_class = args.lcd_class
    
    gaf_df, proteome_to_organism = get_gaf_df(lcd_class)
    prots_df = get_prots(gaf_df, lcd_class, cwd)

    go_obo = 'go-basic.obo'
    go = obo_parser.GODag(os.path.join(cwd, go_obo))
    ...
    for proteome in gaf_df:
        ...
        assoc = get_assoc(cwd, lcd_class, gaf_file)
        background_prots = get_background_prots(proteome, win_size, cwd)

        lcd_prots = prots_df[proteome]
        methods = ["bonferroni", "sidak", "holm", "fdr"]
        organism = organism.replace('(', '').replace(')', '').replace('/', '')
        logfile = open(lcd_class + 'prots_GOruns_'+organism+'_LogFile.txt', 'w')
        g = GOEnrichmentStudy(background_prots, assoc, go, propagate_counts=True, alpha=0.05, log=logfile)
        g_res = g.run_study(lcd_prots)
        filename = proteome + '_' + organism + '_' + lcd_class + 'prots_GOterm_RESULTS.tsv' #change file type here to txt or xlsx if desired
...
def get_gaf_df(lcd_class):

    h = open('GOA_FileMap_' + lcd_class + '_LCDs.tsv')
    df = {}
    df2 = {}
    h.readline()
...
def get_prots(gaf_df, lcd_class, cwd):

    h = open(os.path.join(cwd, lcd_class + '_LCDs_AllOrganisms.tsv'))
    header = h.readline()
    
    df = {}
    
    for line in h:
        proteome, domain, prot_id, *remainder = line.rstrip().split('\t')
        if proteome not in gaf_df:
            continue
...
def get_background_prots(proteome, win_size, cwd):
    
    prots = []
    prot_lengths = {}
    h = open(os.path.join(cwd, 'Eukaryota', proteome + '.fasta'))
...
def get_assoc(cwd, lcd_class, gaf_file):
    """
    ...
    h = open(os.path.join(cwd, 'GOAfiles', lcd_class + '_GOanalysis', gaf_file))
...
def get_args(arguments):
    ...
    parser = argparse.ArgumentParser(description='Run GO-term analysis for select LCD classes in eukaryotes', prog='Select GO')

    parser.add_argument('-c', '--lcd_class', type=str,
                        help="""LCD class to run GO-term analysis for.
                                NOTE: with provided data this is only possible for the HQ, HL, NH, and NM LCD classes.""")
