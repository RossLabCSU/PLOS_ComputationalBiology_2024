...
def main():
    ...
    for proteome in gaf_df:
        ...
        logfile = open('HQprots-WITHOUT-QrichLCD_GOruns_'+organism+'_LogFile.txt', 'w')
        g = GOEnrichmentStudy(background_prots, assoc, go, propagate_counts=True, alpha=0.05, log=logfile)
        g_res = g.run_study(lcd_prots)
        filename = proteome + '_' + organism + '_HQprots-WITHOUT-QrichLCD_GOterm_RESULTS.tsv' #change file type here to txt or xlsx if desired
...
def get_gaf_df():
    ...
    h = open(os.path.join('GOAfiles', 'GOA_FileMap_HQ_LCDs.tsv'))
    df = {}
    df2 = {}
    h.readline()
...
def get_HQprots(gaf_df):
    ...
    h = open('HQprots_Without_QrichLCDs.tsv')
    header = h.readline()
