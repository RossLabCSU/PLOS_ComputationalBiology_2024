...
def main():
    ...
    for directory in ['HQ_GOresults_QrichPrimary_excluded', 'HQ_GOresults_QX-XQ-HX-XH_excluded', 'HQ_GOresults_HrichPrimary_excluded']:
        dirname = os.path.join(cwd, directory)
        
        df = initialize_df()
        org_tracker = {domain:set() for domain in domains}
        ...
        org_counter = {domain:len(org_tracker[domain]) for domain in domains}
        output_data(df, org_counter, directory)


def output_data(df, org_counter, directory):

    output = open(directory + '_GOterm_Summary.tsv', 'w')
