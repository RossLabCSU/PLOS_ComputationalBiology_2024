...
def main():
    ...
    output = open('RUN_Eukaryotes_HQprots_with_QX-XQ-HX-XH_LCDs_Excluded.bat', 'w')
    total_analyses = 0
    for proteome in proteomes_to_analyze:
        if proteome not in proteomes_with_goa_files:
            continue
        
        output.write('python run_GOanalyses_by_Eukaryotes_QX-XQ-HX-XH_excluded.py ' + proteome + '\n')
        total_analyses += 1
